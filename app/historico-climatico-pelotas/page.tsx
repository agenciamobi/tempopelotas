...
export default async function HistoricoClimaticoPelotasPage() {
  ...
  const analyzedDays = hasReliableHistory ? history.days.length : 0;

  const datasetSchema = hasReliableHistory
    ? {
  ...
  return (
    <ForecastPageShell
      ...
      heroStat={{
        ...
        value: hasReliableHistory ? analyzedDays : "Indisponível",
        detail: hasReliableHistory
          ? analyzedDays === 1
            ? "dia analisado"
            : "dias analisados"
          : "nenhum dado demonstrativo exibido",
        ariaLabel: hasReliableHistory
          ? `${analyzedDays} dias analisados no histórico climático recente de Pelotas`
          : "Histórico meteorológico recente temporariamente indisponível",
        tone: "history",
      }}
    >
      {datasetSchema ? (
      ...
      {hasReliableHistory ? (
        <>
          ...
          <section className="history-note" aria-labelledby="history-method-title">
            ...
            <p>
              As informações representam uma estimativa para a região de Pelotas. Elas podem ser diferentes das medições feitas por estações instaladas em pontos específicos da cidade. Fonte: {history.source.name}.
            </p>
          </section>
        </>
      ) : (
        <section className="history-note" aria-labelledby="history-unavailable-title">
          <span className="eyebrow">Dados temporariamente indisponíveis</span>
          <h2 id="history-unavailable-title">O histórico recente não está disponível agora</h2>
          <p>
            O portal não apresenta valores demonstrativos como dados meteorológicos reais. O gráfico e os resumos voltarão a aparecer quando uma fonte confiável ou o arquivo próprio estiver disponível.
          </p>
        </section>
      )}
    </ForecastPageShell>
